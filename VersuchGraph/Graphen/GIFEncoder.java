...
/*******************************************************************************
 * <p>GIFEncoder is a class which takes an image and saves it to a stream
 ...
 *   HREF="http://www.csd.uwo.ca/faculty/wu/cq.c">C source</A>.</li>
 *
 ...
 *   result in the following exception:<br>
 *   <CODE>java.awt.AWTException: Grabber returned false: 192</CODE></li>
 *
 *   <li> The class was modified to constuct with Ury Segal's BMP class,
 *   and it works faster that way</li>
 *
 * </UL><P>
 *
 ...
 */
 ...
 // to get frame image, createImage(), get new image's graphics, pass 
 // graphics to Frame.printAll(g)

 // Support the BMP handling class
...
public class GIFEncoder {
  ...
  /**********************************************************************************/
   ...
   public static void saveGIF(Image im, String name){
     GIFEncoder encoder=null;
      ...
      try{
        encoder.Write(new java.io.FileOutputStream (name+".gif"));
...
/*   public GIFEncoder(BMP image, ColorModel colm) throws AWTException {
  ...
  for ( int h = 0 ; h < height_; h ++ )
    for ( int w = 0 ; w < width_; w ++ )
      pixels_[h*width_ + w ] = ( byte) (image.getPixel(w,h));

  
  // It will cast ; the check is in BMP.writeGIF
  IndexColorModel mcm = (IndexColorModel) colm;

      numColors_ = 1 << BitUtils.BitsNeeded(mcm.getMapSize());
      colors_ = new byte[3*numColors_];

  for ( int i = 0 ; i < mcm.getMapSize() ; i++ ) {
    colors_[i*3] = (byte) mcm.getRed(i);
    colors_[i*3+1] = (byte) mcm.getGreen(i);
    colors_[i*3+2] = (byte) mcm.getBlue(i);
  }
   }
*/
...
//System.out.println("took " + (System.currentTimeMillis()- mystart) + " to grab pixels");
//mystart = System.currentTimeMillis();   
      ...
      int[] fastcolor = new int[256];
      int search, noalpha, colornum = 0;
      byte red, green, blue;
      for (int y = 0; y < height_; ++y)
         for (int x = 0; x < width_; ++x) {
            noalpha = values[index] & 0xFFFFFF;
            r[x][y] = (byte)(noalpha >>> 16);
            g[x][y] = (byte)(noalpha >>> 8);
            b[x][y] = (byte)noalpha;  

            for (search = 0; search < colornum; ++search)
               if (noalpha == fastcolor[search]) break;
            ...
            if (search == colornum) {
               fastcolor[search] = noalpha;
               colors_[(search = search * 3)] = r[x][y];
               colors_[++search] = g[x][y];
               colors_[++search] = b[x][y];
               ++colornum;
            }
            ++index;
         }

//System.out.println("took " + (System.currentTimeMillis()- mystart) + " to break up int");
...
//System.out.println("took " + (System.currentTimeMillis()- mystart) + " to LZW");
...
class ScreenDescriptor {
   ...
   public void SetGlobalColorTableSize(byte num) { byte_ |= (num & 7); }
   public void SetSortFlag            (byte num) { byte_ |= (num & 1) << 3; }
   public void SetColorResolution     (byte num) { byte_ |= (num & 7) << 4; }
   public void SetGlobalColorTableFlag(byte num) { byte_ |= (num & 1) << 7; }
}
...
class ImageDescriptor {
   ...
   public void SetLocalColorTableSize(byte num) { byte_ |= (num & 7); }
   public void SetReserved           (byte num) { byte_ |= (num & 3) << 3; }
   public void SetSortFlag           (byte num) { byte_ |= (num & 1) << 5; }
   public void SetInterlaceFlag      (byte num) { byte_ |= (num & 1) << 6; }
   public void SetLocalColorTableFlag(byte num) { byte_ |= (num & 1) << 7; }
}
